...
class ImageComponent extends Object 
{
    ...
    */
    ...
    function upload_image_and_thumbnail($data, $datakey, $imgscale, $thumbscale, $folderName, $square) {
        if (strlen($data['Image']['name'])>4){
                    $error = 0;
                    $tempuploaddir = "img".DS."temp"; // the /temp/ directory, should delete the image after we upload
                    $biguploaddir = "img".DS.$folderName.DS."big"; // the /big/ directory
                    $smalluploaddir = "img".DS.$folderName.DS."small"; // the /small/ directory for thumbnails
                    ...
                    $filetype = $this->getFileExtension($data['Image']['name']);
                    ...
                    {
                        // Get the image size
                        $imgsize = GetImageSize($data['Image']['tmp_name']);
                    }
    ...
    */
    function delete_image($filename,$folderName) {
        unlink("img".DS.$folderName.DS."big".DS.$filename);
        unlink("img".DS.$folderName.DS."small".DS.$filename);
    }
